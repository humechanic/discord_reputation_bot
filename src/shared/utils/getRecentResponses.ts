...
export const getRecentResponses = async (reaction, emoji, type = BOT_RESPONSE_TYPE.AUTO) => {
    ...
    const recentResponses = existingMessages.filter(msg => msg.author.id === discordClient.user!.id && (type === BOT_RESPONSE_TYPE.REPUTATION ? msg.content.includes('increased') : msg.content === EMOJI_MESSAGES_MAP.get(emoji)) && msg.createdTimestamp >= currentDate.getTime());
